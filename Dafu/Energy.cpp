...
{
	KF.transitionMatrix = (get_diagonal_mat(state_num));// 转移矩阵
	setIdentity(KF.measurementMatrix);					// 测量矩阵
	setIdentity(KF.processNoiseCov, Scalar::all(q));	// 系统噪声方差矩阵
	setIdentity(KF.measurementNoiseCov, Scalar::all(R));// 测量噪声方差矩阵
	setIdentity(KF.errorCovPost, Scalar::all(1));		// 误差矩阵
...
bool Energy::judgeRPosition(RotatedRect &R, ArmorRect &present, ArmorRect &former)
{
	...
	if (vec.getNorm() / present.long_edge > 5 || vec.getNorm() / present.long_edge <2.5) // 3.2左右
	{
		return false;
	}
